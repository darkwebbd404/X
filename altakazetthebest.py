...
def talk_with_ai(question):
    ...
    if res.status_code == 200:
        ...
        return "Terjadi kesalahan saat menghubungkan ke server."
...
def generate_random_word():
    word_list = [
        "ZUY", "ZUYXXX", "ZUYXXX TEAM.", "ZUYXXX", "ZUYXXX TEAM",
        "tt : @ZuyXx_", "ZUYXXX", "ZUYXXX", "tt : @ZuyXx_", "tt : @ZuyXx_",
        "tt : @ZuyXx_", "tt : @ZuyXx_", "DONE_HACK", "tt : @ZuyXx_", "tt : @ZuyXx_" 
]
    ...
    def connect(self, tok, host, port, packet, key, iv):
        ...
        while True:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""[C][B]Assalamu'alaikum, apa kabar? Terima kasih atas penerimaan Anda.""", idinv
                        )
                )
                senthi = False            
            if "1200" in data.hex()[0:4]:
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                    	...
                    	response_message =f"""{generate_random_color()}WELCOME TO ALTAKZ X BOT VIIP!

{color}I Heard You Wanna See Who's The Admins?

{color}I Feel Like You Wanna Buy Admin

{color}Or At Least You Wanna Buy A Server With Your Name

{color}Right?, Well You Can!

{generate_random_color()}Telegram:🗿@ZuyXxx - @pioneastro

{generate_random_color()} Discord:🦋@snowfwal

{color}Nikmati botnya terima kasih! 

{color}Also, You Can Send /🦋help To Know My Commands!

{generate_random_color()}Bot Made By 🦋@jetskyy"""
...
{l}""",uid))
                    ...
                    try:
                        ...
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                        clients.send(self.GenResponsMsg("[FF0000][b]❗ Terjadi kesalahan dalam operasi[/b]",uid))
                    except:
                        restart_program()
################################
            ...
            if "1200" in data.hex()[0:4] and b"/6s" in data:
                ...
                if iddd:
	                clients.send(
	                    self.GenResponsMsg(
	                        f"""
[C][B][00ff00]Please accept invite quickly ! 
	                        """, iddd
	                    )
	                )
                sleep(5)
...
{l}""", uid))
                    ...
                    try:
                        ...
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                        error_msg = f"[FF0000] KESALAHAN: {e}" if "ID" in str(e) else f"[FF0000]Terjadi kesalahan saat mengambil pemain: {e}"
...
################################
            if "1200" in data.hex()[0:4] and b"/ai" in data:
                try:
                    ...
                    clients.send(self.GenResponsMsg("[FFFFFF]Processing Wait....", uid))
                	...
                	if room_data and len(room_data) > 0:
                    		...
                    		if player_id.isdigit():
                        		...
                        		if "IN ROOM" in tempdata:
                            			...
                            			clients.send(
                                self.GenResponsMsg(
                                    f"\n{generate_random_color()}- SPAM START FOR ID {fix_num(player_id)} !\n", uid
                                )
                            )
                                ...
                                self.GenResponsMsg(
                    		      ...
                    		      clients.send(
                            self.GenResponsMsg(
                                f"\n\n\n[C][B] [FF00FF]Please write the id of player notTolong tulis id pemain bukan!\n\n\n", uid
                            )
                        )   
                	else:
                	       clients.send(
...
################################
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:	                
	                ...
	                uid = parsed_data["5"]["data"]["1"]["data"]
	                color = generate_random_color()
	                response_message =f"""{generate_random_color()}WELCOME TO ALTAKZ VIP!)

[00FF00][b][c]To create a squad:
[00FF00][b][c]/🗿3s
[00FF00][b][c]/🗿5s
[00FF00][b][c]/🗿6s
[00FF00][b][c]To open a squad for your friend:
[00FF00][b][c]/🗿snd/104[c]145[c]933[c]49 5

[00FF00][b][c]To invite someone by ID:
[00FF00][b][c]/🗿inv/104[c]145[c]933[c]49 5

[00FF00][b][c]To spam a room:
[00FF00][b][c]/🗿room/104[c]145[c]933[c]49

[00FF00][b][c]Get player info:
[00FF00][b][c]/🗿info/104[c]145[c]933[c]49

[00FF00][b][c]Check player status:
[00FF00][b][c]/🗿status/104[c]145[c]933[c]49

[00FF00][b][c]Check if account is banned:
[00FF00][b][c]/🗿check/

[00FF00][b][c]Add 100 likes:
[00FF00][b][c]/🗿likes/104[c]145[c]933[c]49

[00FF00][b][c]Talk to ChatGPT:
[00FF00][b][c]/🗿ai [your message here]

{generate_random_color()}Sosmed Dev:
TG: @pioneastro-@jetsskyy
DC: @snowfwal
{generate_random_color()}Developed by: altakz"""
	                clients.send(self.GenResponsMsg(response_message, uid))
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3996475093", password="2002CADCC5616A07B9B091E4DF225C454BB94F0ECB9176762DC0EC5BF183A88A")
        client_thread.start()
    except Exception as e:
